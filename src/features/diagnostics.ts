...
export function activateDiagnostics(context: vscode.ExtensionContext) {
	...
	const scheduleValidate = (doc: vscode.TextDocument) => {
		if (timer) {clearTimeout(timer);}
		timer = setTimeout(() => runValidate(doc), 300);
	};

	const runValidate = async (doc: vscode.TextDocument) => {
		if (doc.languageId !== 'gptp') { return; }
		...
		try {
			...
			const declared = new Set<string>((inspect?.variables || []).map((v: any) => v.name || v));
		...
		collection.set(doc.uri, diagnostics);
			...
			context.subscriptions.push(vscode.workspace.onDidOpenTextDocument((doc: vscode.TextDocument) => scheduleValidate(doc)));
			context.subscriptions.push(vscode.workspace.onDidChangeTextDocument((e: vscode.TextDocumentChangeEvent) => scheduleValidate(e.document)));
		// Also revalidate on save to ensure stale diagnostics are cleared automatically
		context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc: vscode.TextDocument) => scheduleValidate(doc)));
		...
		context.subscriptions.push(vscode.workspace.onDidCloseTextDocument((doc: vscode.TextDocument) => collection.delete(doc.uri)));
...
function approximateRangeFromPointer(text: string, pointer: string): { start: number; end: number } | undefined {
	try {
		...
		if (ch === '"') {
			...
			if (end > valueStart) {return { start: valueStart, end };}
		}
...
function findFirstVarUsageRange(text: string, varName: string): { start: number; end: number } | undefined {
	try {
...
function findVarDeclarationRange(text: string, varName: string): { start: number; end: number } | undefined {
	try {
		...
		if (arrEnd <= arrStart) {return undefined;}
